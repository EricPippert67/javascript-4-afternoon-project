...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating = function(number){ 
    return this.rating = (number + this.rating) /2
  }
  ...
  // Name and email will be strings, age will be a number and savedPosts will be an array of objects. These objects will each have 3 properties: id (a number), 
  // title (a string), and rating (a number between 1 and 5). These objects are the posts that the user will have saved to their account.
  
  // // Once the User constructor function is created, write a prototype method for the User function. Name this method addSavedPost. It should take in three 
  // parameters: id (a number), title (a string) and rating (a number). Use these parameters to create a new object and add it to the savedPosts array. Make sure to name 
  // the properties the same as described previously (id, title, rating).
  ...
  // // Write a prototype method for the User constructor function named removeSavedPost that will take in one number parameter representing the post id. Use this id to
  //  find and remove the matching object in the savedPosts array.
  
  User.prototype.removeSavedPost = function(id) {
    this.savedPosts = this.savedPosts.filter(function(element){
  ...
  // // Write a prototype method for the User constructor function named changePostRating that will take in two number parameters. The first will be an id (a number)
  //  and the second will be the new rating (a number). Use the id to find the matching object in the savedPosts array. Once you find the matching object, update it's 
  //  rating score with the new rating parameter.
  
  User.prototype.changePostRating = function(id, num){
    this.savedPosts.map(element => element.id === id ? element.rating = num: null)
  }